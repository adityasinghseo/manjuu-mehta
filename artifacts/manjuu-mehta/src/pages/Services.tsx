      ...
      <section className="py-28 bg-primary text-primary-foreground text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeIn} className="max-w-2xl mx-auto space-y-8">
            <p className="text-secondary font-medium uppercase tracking-widest text-sm">Begin Today</p>
            <h2 className="text-4xl md:text-6xl font-serif font-bold">
              Speak directly with expert now and get instant guidance
            </h2>
            <p className="text-lg text-primary-foreground/75">No forms, no waiting — just a real conversation that changes everything.</p>
            <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
              <Button asChild size="lg" className="bg-secondary hover:bg-secondary/90 text-secondary-foreground rounded-full px-10 py-6 text-lg h-auto font-semibold">
                <a href={CALL_LINK}><Phone className="w-5 h-5 mr-2" />Call Now</a>
              </Button>
              <Button asChild size="lg" className="bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-full px-10 py-6 text-lg h-auto font-semibold">
                <a href={WA_LINK} target="_blank" rel="noopener noreferrer"><MessageCircle className="w-5 h-5 mr-2" />WhatsApp Now</a>
              </Button>
            </div>
          </motion.div>
        </div>
